...
// [[Rcpp::export]]
Eigen::MatrixXd K1_D_nu(int k, Eigen::MatrixXd D, Eigen::MatrixXd Q,
                        NumericVector dim, NumericVector C, NumericVector Y,
                        int jj, int nu = 0) {
  // this function is adapted to data with K=1 for each time point
  ...
  for (int i=0; i < dim1; i++){
    for (int j=0; j < dim2; j++){
      tensor(i,j) = Y(i + j*dim1);
    }
  }
  
  Eigen::Tensor<double, 2> C_tensor(dim1, dim2);
  
  for (int i=0; i < dim1; i++){
    for (int j=0; j < dim2; j++){
      C_tensor(i,j) = C(i + j*dim1);
    }
  }
